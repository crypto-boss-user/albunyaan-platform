...
| 2 Baseline | ja | 09:48:09–09:48:15 | hoofdsessie | `node --check` HEAD OK + 91a5c1c OK · `worker: tsc --noEmit` **rc=2, pre-existing fouten** — op 04-09 gelogd als "3" (`playwright-core`-types in `uscreen-login.ts:19`, `uscreen-scraper.ts:16,56`), maar dat was een `tail -8`-meetartefact: de volledige telling is **14 fouten in 8 bestanden** (DOM-types in `page.evaluate`-callbacks + `playwright-core`-types; correctie RV 2, 2026-09-04). `.mjs` valt buiten `include: **/*.ts`, dus geen daarvan raakt dit bestand · `vitest run` **5 bestanden, 35/35 pass, 1,18 s**. Baseline die niet mag regresseren: 35/35 + 14 tsc-fouten (niet meer; gecorrigeerd van 3) |
