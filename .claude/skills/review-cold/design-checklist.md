...
**Trigger:** Only run this checklist if the diff touches frontend files. Bepaal handmatig of de diff frontend-bestanden raakt (gstack-diff-scope n.v.t. lokaal):

```bash
# gstack-diff-scope n.v.t. lokaal: bepaal SCOPE_FRONTEND handmatig uit deze --stat
...
**"zou AUTO-FIX zijn"** (mechanical CSS fixes — lokaal tóch ASK, de reviewer bewerkt nooit):
...
**LOW confidence items** → present as "Possible: [description]. Verify visually or run /design-review." Nooit bewerken (lokaal: alles ASK).
...
```
Design Review: N issues (X klein/mechanisch, Y oordeel, Z possible) — alle ASK
