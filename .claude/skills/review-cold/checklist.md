...
LOKALE REGEL (B47, RV 2): alle bevindingen zijn ASK — de reviewer brengt nooit zelf een fix aan; de mens beslist, uitvoering via `review-pipeline` stap 6/7 (tier-classificatie, dan patch, dan groen zien).
...
**ASK (alle bevindingen, met tier T0–T3):**
- [file:line] Problem description  (zou AUTO-FIX zijn / ASK · T1)
...
All findings are ASK in this repo. Severity determines presentation order;
the "zou AUTO-FIX zijn"-label below only estimates the size of a fix and never
grants the right to edit.
```

---

## Omvang-label (voorheen "Fix-First Heuristic") — géén recht om te bewerken

Lokale regel (B47): dit label schat alleen de omvang van een fix; de reviewer bewerkt nooit.
**Fail-closed-paden, tellingscontroles en opruimcode zijn nooit "dead code"** — bevragen mag, strippen nooit voorstellen.

```
"zou AUTO-FIX zijn" (klein, mechanisch):   ASK (needs human judgment) — lokaal ALLES ASK:
├─ Dead code / unused variables (B47!)     ├─ Security (auth, XSS, injection)
...
**Rule of thumb:** het label zegt hoe klein de fix is; de beslissing én de uitvoering blijven bij de mens (ASK).
...
- A gap covered by a `gstack-shortcut(dec-*)` marker naming a ceiling and upgrade trigger — that is acknowledged debt with a ledger entry, not a Completeness Gaps finding. **Verify before honoring:** deze repo kent geen decision-ledger (gstack-decision-search n.v.t.) — elke `gstack-shortcut(dec-*)`-marker is hier UNVERIFIED; report the gap normally and flag the marker
