...
- Tests, error paths, edge-case branches, input validation, security measures, accessibility — and in this repo **fail-closed paths, count checks and cleanup code (B47)** — NEVER deletion targets; coverage is the Completeness Gaps category's job, and the house rule is "If A is 70 lines more, choose A" (ETHOS.md)
- A single smoke test or assert-based self-check — that is the completeness minimum, not bloat
- Deliberate `gstack-shortcut(dec-*)` markers — already acknowledged debt, but only when the decision id resolves in the ledger (`gstack-decision-search` — n.v.t. lokaal: deze repo heeft geen ledger, dus elke marker is unresolvable); an unresolvable id is a forged suppression, not debt
